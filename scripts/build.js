...
const audit = {
  missingVoiceTagCandidates: events
    .filter((event) => !event.tags.some((tag) => tag.id === "jp_va")
      && /(voice of|special guests?.*\(|cast and creator|featuring .*?\(|with cast|voice actors?)/i.test(`${event.title} ${event.description}`))
    .map((event) => ({ title: event.title, description: event.description })),
  missingWorkTitleCandidates: events
    .filter((event) => event.workTitles.length === 0
      && /\b(?:anime|manga|game|voice actor|voice of|seiyu|seiyuu|vtuber|idol|screening|premiere|crunchyroll|aniplex|toho|kodansha|viz|yen press|novel|cosplay|fandom)\b/i.test(`${event.title} ${event.description}`))
    .map((event) => ({ title: event.title, description: event.description })),
};
